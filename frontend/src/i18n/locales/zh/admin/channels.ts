export default {
    ...
    channelMonitor: {
      ...
      visibility: {
        title: '用户侧渠道状态可见性',
        description: '控制哪些用户能看到「渠道状态」。管理员始终可访问，不受此限制。',
        all: '全员可见',
        allHint: '所有已登录用户都可以打开渠道状态。',
        selected: '仅部分用户可见',
        selectedHint: '仅名单中的用户可以看到渠道状态；未选中的用户入口隐藏、接口不可用。管理员不受限制。',
        userSearchPlaceholder: '搜索用户邮箱…',
        userSearchEmpty: '没有匹配的用户',
        userDeleted: '已删除',
        userIdFallback: '用户 #{id}',
        removeUser: '移除',
        save: '保存可见性',
        saveSuccess: '渠道状态可见性已保存',
        saveFailed: '保存可见性失败',
        saveNotPersisted: '服务器未保存可见性设置，请确认后端已更新后再试',
        loadFailed: '加载可见性设置失败',
        selectedCount: '已选择 {count} 位用户',
        emptySelected: '尚未选择用户；当前除管理员外无人可见。',
      },
      advanced: {
        section: '高级（可选）',
