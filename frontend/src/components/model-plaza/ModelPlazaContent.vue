  ...
  <section class="model-plaza" :class="`model-plaza--${variant}`">
    ...
    <div v-else-if="view === 'card'" class="model-plaza__grid">
      <article v-for="entry in filteredEntries" :key="`${entry.groupId}:${entry.id}`" class="model-plaza__card">
        ...
        <div v-if="entry.intervals.length" class="model-plaza__context-prices">
          <div v-for="interval in entry.intervals" :key="interval.label" class="model-plaza__context-tier">
            <div class="model-plaza__context-head"><span>{{ text.longContext }}</span><strong>{{ interval.label }}</strong></div>
            <div class="model-plaza__context-grid"><span><small>{{ text.input }}</small><em>${{ formatPrice(interval.input) }}</em><strong>${{ formatActualPrice(interval.input, entry.rateMultiplier) }}</strong></span><span><small>{{ text.cache }}</small><em>${{ formatPrice(interval.cache) }}</em><strong>${{ formatActualPrice(interval.cache, entry.rateMultiplier) }}</strong></span><span><small>{{ text.output }}</small><em>${{ formatPrice(interval.output) }}</em><strong>${{ formatActualPrice(interval.output, entry.rateMultiplier) }}</strong></span></div>
          </div>
        </div>
    ...
    <div v-else class="model-plaza__table-wrap">
      <table class="model-plaza__table"><thead><tr><th>{{ text.model }}</th><th>{{ text.rate }}</th><th>{{ text.group }}</th><th>{{ text.input }}</th><th>{{ text.cache }}</th><th>{{ text.output }}</th><th>{{ text.unit }}</th></tr></thead><tbody><tr v-for="entry in filteredEntries" :key="`${entry.groupId}:${entry.id}`"><td><div class="model-plaza__table-model"><span class="model-plaza__provider-logo" :class="entry.provider"><ModelIcon :model="entry.provider" size="20px" /></span><span><code>{{ entry.id }}</code><small>{{ providerLabel(entry.provider) }}</small><small v-for="interval in entry.intervals" :key="interval.label" class="model-plaza__table-tier">{{ text.longContext }} {{ interval.label }}</small></span></div></td><td><span class="model-plaza__rate">{{ formatRate(entry.rateMultiplier) }}</span></td><td>{{ entry.groupName }}</td><td><span class="model-plaza__table-original">{{ text.original }} ${{ formatPrice(entry.input) }}</span><strong class="model-plaza__table-actual">{{ text.actual }} ${{ formatActualPrice(entry.input, entry.rateMultiplier) }}</strong><span v-for="interval in entry.intervals" :key="interval.label" class="model-plaza__table-interval"><small>{{ interval.label }}</small>{{ text.original }} ${{ formatPrice(interval.input) }}<strong>{{ text.actual }} ${{ formatActualPrice(interval.input, entry.rateMultiplier) }}</strong></span></td><td><span class="model-plaza__table-original">{{ text.original }} ${{ formatPrice(entry.cache) }}</span><strong class="model-plaza__table-actual">{{ text.actual }} ${{ formatActualPrice(entry.cache, entry.rateMultiplier) }}</strong><span v-for="interval in entry.intervals" :key="interval.label" class="model-plaza__table-interval"><small>{{ interval.label }}</small>{{ text.original }} ${{ formatPrice(interval.cache) }}<strong>{{ text.actual }} ${{ formatActualPrice(interval.cache, entry.rateMultiplier) }}</strong></span></td><td><span class="model-plaza__table-original">{{ text.original }} ${{ formatPrice(entry.output) }}</span><strong class="model-plaza__table-actual">{{ text.actual }} ${{ formatActualPrice(entry.output, entry.rateMultiplier) }}</strong><span v-for="interval in entry.intervals" :key="interval.label" class="model-plaza__table-interval"><small>{{ interval.label }}</small>{{ text.original }} ${{ formatPrice(interval.output) }}<strong>{{ text.actual }} ${{ formatActualPrice(interval.output, entry.rateMultiplier) }}</strong></span></td><td>USD / 1M Token</td></tr></tbody></table>
    </div>
