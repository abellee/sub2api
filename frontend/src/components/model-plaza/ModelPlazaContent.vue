...
function formatListedPrice(value: string | null) { const formatted = formatPrice(value); return formatted === '-' ? '-' : `$${formatted}` }
function formatListedActual(value: string | null, rateMultiplier: number) { const formatted = formatActualPrice(value, rateMultiplier); return formatted === '-' ? '-' : `$${formatted}` }
function unitLabel(entry: ModelPlazaEntry) { return entry.kind === 'image' ? text.value.imageUnit : entry.kind === 'video' ? text.value.videoUnit : text.value.tokenUnit }
...
<template>
  <section class="model-plaza" :class="`model-plaza--${variant}`">
    ...
    <div v-else-if="view === 'card'" class="model-plaza__grid">
      <article v-for="entry in filteredEntries" :key="`${entry.groupId}:${entry.id}`" class="model-plaza__card">
        ...
        <template v-if="entry.kind === 'token'">
          <div class="model-plaza__prices"><span><small>{{ text.input }}</small><span class="model-plaza__original-price"><i>{{ text.original }}</i><em>{{ formatListedPrice(entry.input) }}</em></span><span class="model-plaza__actual-price"><i>{{ text.actual }}</i><strong>{{ formatListedActual(entry.input, entry.rateMultiplier) }}</strong></span></span><span><small>{{ text.cache }}</small><span class="model-plaza__original-price"><i>{{ text.original }}</i><em>{{ formatListedPrice(entry.cache) }}</em></span><span class="model-plaza__actual-price"><i>{{ text.actual }}</i><strong>{{ formatListedActual(entry.cache, entry.rateMultiplier) }}</strong></span></span><span><small>{{ text.output }}</small><span class="model-plaza__original-price"><i>{{ text.original }}</i><em>{{ formatListedPrice(entry.output) }}</em></span><span class="model-plaza__actual-price"><i>{{ text.actual }}</i><strong>{{ formatListedActual(entry.output, entry.rateMultiplier) }}</strong></span></span></div>
          <div v-if="entry.tokenTiers.length" class="model-plaza__context-prices">
            <div v-for="tier in entry.tokenTiers" :key="tier.label" class="model-plaza__context-tier">
              ...
              <div class="model-plaza__context-grid">
                <span><small>{{ text.input }}</small><em>{{ text.original }} {{ formatListedPrice(tier.input) }}</em><strong>{{ text.actual }} {{ formatListedActual(tier.input, entry.rateMultiplier) }}</strong></span>
                <span><small>{{ text.cache }}</small><em>{{ text.original }} {{ formatListedPrice(tier.cache) }}</em><strong>{{ text.actual }} {{ formatListedActual(tier.cache, entry.rateMultiplier) }}</strong></span>
                <span><small>{{ text.output }}</small><em>{{ text.original }} {{ formatListedPrice(tier.output) }}</em><strong>{{ text.actual }} {{ formatListedActual(tier.output, entry.rateMultiplier) }}</strong></span>
              </div>
            </div>
          </div>
        </template>
        <div v-else class="model-plaza__context-prices">
          <div class="model-plaza__context-tier">
            ...
            <div class="model-plaza__context-grid">
              <span v-for="tier in entry.tiers" :key="tier.label"><small>{{ tier.label || (entry.kind === 'image' ? text.perImage : text.perVideo) }}</small><em>{{ text.original }} {{ formatListedPrice(tier.original) }}</em><strong>{{ text.actual }} {{ formatListedActual(tier.original, entry.rateMultiplier) }}</strong></span>
            </div>
          </div>
        </div>
    ...
    <div v-else class="model-plaza__table-wrap">
      <table class="model-plaza__table"><thead><tr><th>{{ text.model }}</th><th>{{ text.rate }}</th><th>{{ text.group }}</th><th>{{ text.input }}</th><th>{{ text.cache }}</th><th>{{ text.output }}</th><th>{{ text.unit }}</th></tr></thead><tbody><tr v-for="entry in filteredEntries" :key="`${entry.groupId}:${entry.id}`"><td><div class="model-plaza__table-model"><span class="model-plaza__provider-logo" :class="entry.provider"><ModelIcon :model="entry.provider" size="20px" /></span><span><code>{{ entry.id }}</code><small>{{ providerLabel(entry.provider) }}</small></span></div></td><td><span class="model-plaza__rate">{{ formatRate(entry.rateMultiplier) }}</span></td><td>{{ entry.groupName }}</td><template v-if="entry.kind === 'token'"><td><span class="model-plaza__table-original">{{ text.original }} {{ formatListedPrice(entry.input) }}</span><strong class="model-plaza__table-actual">{{ text.actual }} {{ formatListedActual(entry.input, entry.rateMultiplier) }}</strong><span v-for="tier in entry.tokenTiers" :key="tier.label" class="model-plaza__table-interval"><small>{{ text.longContext }} · {{ tier.label }}</small><span>{{ text.original }} {{ formatListedPrice(tier.input) }}</span><strong>{{ text.actual }} {{ formatListedActual(tier.input, entry.rateMultiplier) }}</strong></span></td><td><span class="model-plaza__table-original">{{ text.original }} {{ formatListedPrice(entry.cache) }}</span><strong class="model-plaza__table-actual">{{ text.actual }} {{ formatListedActual(entry.cache, entry.rateMultiplier) }}</strong><span v-for="tier in entry.tokenTiers" :key="tier.label" class="model-plaza__table-interval"><small>{{ text.longContext }} · {{ tier.label }}</small><span>{{ text.original }} {{ formatListedPrice(tier.cache) }}</span><strong>{{ text.actual }} {{ formatListedActual(tier.cache, entry.rateMultiplier) }}</strong></span></td><td><span class="model-plaza__table-original">{{ text.original }} {{ formatListedPrice(entry.output) }}</span><strong class="model-plaza__table-actual">{{ text.actual }} {{ formatListedActual(entry.output, entry.rateMultiplier) }}</strong><span v-for="tier in entry.tokenTiers" :key="tier.label" class="model-plaza__table-interval"><small>{{ text.longContext }} · {{ tier.label }}</small><span>{{ text.original }} {{ formatListedPrice(tier.output) }}</span><strong>{{ text.actual }} {{ formatListedActual(tier.output, entry.rateMultiplier) }}</strong></span></td></template><td v-else colspan="3"><div class="model-plaza__table-media"><span v-for="tier in entry.tiers" :key="tier.label" class="model-plaza__media-chip"><small>{{ tier.label || (entry.kind === 'image' ? text.perImage : text.perVideo) }}</small><span class="model-plaza__table-original">{{ text.original }} {{ formatListedPrice(tier.original) }}</span><strong class="model-plaza__table-actual">{{ text.actual }} {{ formatListedActual(tier.original, entry.rateMultiplier) }}</strong></span></div></td><td>{{ unitLabel(entry) }}</td></tr></tbody></table>
    </div>
