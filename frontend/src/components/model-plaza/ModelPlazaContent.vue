        ...
        <div v-else class="model-plaza__context-prices">
          <div class="model-plaza__context-tier">
            ...
            <div class="model-plaza__context-grid">
              <span v-for="tier in entry.tiers" :key="tier.label"><small>{{ tier.label }}</small><em>{{ text.original }} {{ formatListedPrice(tier.original) }}</em><strong>{{ text.actual }} {{ formatListedActual(tier.original, entry.rateMultiplier) }}</strong></span>
            </div>
          </div>
        </div>
