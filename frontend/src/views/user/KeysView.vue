...
watch([showCreateModal, createProviderOptions], ([isOpen, providers], [wasOpen]) => {
  if (!isOpen) return
  const selectedGroup = groupOptions.value.find((group) => group.value === formData.value.group_id)
  const selectedProvider = selectedGroup ? getKeyGroupProvider(selectedGroup.platform) : null
  if (!wasOpen) {
    if (selectedProvider && providers.some((provider) => provider.value === selectedProvider && provider.count > 0)) {
      createProvider.value = selectedProvider
    } else {
      selectCreateProvider(providers.find((provider) => provider.count > 0)?.value ?? 'anthropic')
    }
  } else if (!providers.some((provider) => provider.value === createProvider.value && provider.count > 0)) {
    selectCreateProvider(providers.find((provider) => provider.count > 0)?.value ?? 'anthropic')
  }
...
const selectRecommendedGroup = (groupId: number) => {
  const group = groupOptions.value.find((option) => option.value === groupId)
  if (group) createProvider.value = getKeyGroupProvider(group.platform)
  formData.value.group_id = groupId
}
