    ...
    <div class="card-header mb-4 flex shrink-0 flex-wrap items-start justify-between gap-3 !border-0 !p-0">
      <div class="min-w-0">
        <h2 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
          <span class="inline-flex h-4 w-4 text-indigo-500" aria-hidden="true">
      ...
      <div class="flex w-full min-w-0 flex-wrap items-center justify-end gap-2 text-xs text-gray-500 dark:text-gray-400 sm:w-auto">
        <span class="inline-flex items-center gap-1.5 rounded-full bg-rose-50 px-2 py-0.5 text-[11px] font-medium text-rose-600 dark:bg-rose-950/40 dark:text-rose-300">
          <span class="h-1.5 w-1.5 rounded-full bg-rose-500"></span>{{ t('channelMonitorV2.chart.errorLegend') }}
        </span>
        <span class="inline-flex items-center gap-1.5 rounded-full bg-teal-50 px-2 py-0.5 text-[11px] font-medium text-teal-700 dark:bg-teal-950/40 dark:text-teal-300">
          <span class="h-1.5 w-1.5 rounded-full bg-teal-500"></span>{{ t('channelMonitorV2.chart.cacheLegend') }}
        </span>
        <span class="inline-flex items-center gap-1.5 rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-medium text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300">
          <span class="h-1.5 w-1.5 rounded-full bg-indigo-500"></span>{{ t('channelMonitorV2.chart.ttftLegend') }}
        </span>
...
const chartOptions = computed(() => {
  const text = isDark.value ? '#9ca3af' : '#78716c'
  const grid = isDark.value ? '#374151' : '#f3eee6'
  const tooltipBg = isDark.value ? '#1f2937' : '#fffdf8'
  const tooltipTitle = isDark.value ? '#f3f4f6' : '#1c1917'
  const tooltipBody = isDark.value ? '#d1d5db' : '#57534e'
  return {
    responsive: true,
