  ...
  >
    <div class="card-header mb-4 flex shrink-0 flex-wrap items-start justify-between gap-3 !border-0 !p-0">
      <div class="min-w-0">
        <h2 class="flex items-center gap-2 text-sm font-bold text-gray-900 dark:text-white">
          <span class="inline-flex h-4 w-4 text-teal-500" aria-hidden="true">
      ...
      <div class="flex w-full min-w-0 flex-wrap items-center justify-end gap-2 text-xs text-gray-500 dark:text-gray-400 sm:w-auto">
        ...
        <button
          type="button"
          class="btn btn-secondary btn-sm !px-2 !py-1 text-[11px]"
          :disabled="!zoomed"
    ...
    <div class="card-body min-h-0 flex-1 !p-0">
      <div
        v-if="rows.length"
        ref="scrollRef"
        class="studio-pulse-scroll max-h-[min(42vh,420px)] max-w-full overflow-auto rounded-2xl bg-[#F6F1E8]/70 p-3 dark:bg-dark-900/30"
        @wheel="onMatrixWheel"
      >
        <div class="studio-pulse-table w-full" :style="tableStyle">
          <div
            class="studio-pulse-header studio-pulse-row sticky top-0 z-[3] bg-[#F6F1E8] text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:bg-dark-900 dark:text-gray-400"
            :class="showThroughput ? 'studio-pulse-row--with-tps' : ''"
          >
          ...
          <div
            ...
            class="studio-pulse-row"
            :class="showThroughput ? 'studio-pulse-row--with-tps' : ''"
          >
            <div class="flex min-w-0 items-center gap-2" :title="rowLabel(entry.row)">
              <span :class="['studio-dot', cellClass(entry.row.health, entry.row.metrics.request_count)]"></span>
              <strong class="truncate text-xs font-semibold text-gray-800 dark:text-gray-100">{{ rowLabel(entry.row) }}</strong>
            </div>
            <strong class="text-xs font-medium tabular-nums text-gray-600 dark:text-gray-300">
              {{ successRate(entry.row.metrics) }}
            </strong>
            <strong
              class="text-xs font-medium tabular-nums text-gray-600 dark:text-gray-300"
            ...
            <div class="studio-pulse-track grid items-stretch" :style="pulseStyle">
              <span
                v-for="slot in entry.slots"
                :key="slot.start"
                class="studio-pulse-cell"
                :class="[
      ...
      <div class="mt-4 flex flex-col gap-2" :aria-label="t('channelMonitorV2.matrix.legendAria')">
        <div class="flex items-center gap-2 text-[11px] text-gray-500 dark:text-gray-400">
          ...
          <div class="studio-score-legend h-2.5 flex-1 overflow-hidden rounded-full"></div>
        ...
        <div class="flex flex-wrap gap-4 text-[11px] text-gray-500 dark:text-gray-400">
          <span class="inline-flex items-center gap-1.5"><i class="studio-dot health-score10"></i>{{ t('channelMonitorV2.matrix.healthyLegend') }}</span>
          <span class="inline-flex items-center gap-1.5"><i class="studio-dot health-score6"></i>{{ t('channelMonitorV2.matrix.warningLegend') }}</span>
          <span class="inline-flex items-center gap-1.5"><i class="studio-dot health-score2"></i>{{ t('channelMonitorV2.matrix.criticalLegend') }}</span>
          <span class="inline-flex items-center gap-1.5"><i class="studio-dot health-unknown"></i>{{ t('channelMonitorV2.matrix.unknownLegend') }}</span>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      ...
      >
        <span
          ...
          :key="`${index}:${line}`"
          class="studio-pulse-tooltip-line"
          :class="index === 0 ? 'studio-pulse-tooltip-title' : ''"
        >
          {{ line }}
        </span>
      </div>
...
const pulseStyle = computed(() => {
  ...
  const gapPx = !zoomed.value ? (count > 24 ? 2 : 3) : intensity >= 4 ? 3 : 2
  const minCell = !zoomed.value ? '0' : `${8 + intensity * 4}px`
  return {
...
function onMatrixWheel(event: WheelEvent) {
  ...
  const pulse = target?.closest('.studio-pulse-track') as HTMLElement | null
  const overMatrix = Boolean(target?.closest('.studio-pulse-scroll'))
...
.health-unknown  { background: #d6d3d1; }
...
.studio-pulse-cell.has-data {
  cursor: help;
}
.studio-pulse-cell.is-empty {
  opacity: 0.45;
}
.studio-pulse-cell.has-data:hover,
.studio-pulse-cell.has-data:focus-visible {
  outline: 2px solid rgb(20 184 166 / 0.55);
  outline-offset: 1px;
  z-index: 5;
}

.studio-pulse-tooltip {
  pointer-events: none;
  position: fixed;
  z-index: 9999;
  min-width: 11.5rem;
...
:global(.dark) .studio-pulse-tooltip {
  border-color: rgb(55 65 81);
  background: rgb(17 24 39);
  color: rgb(229 231 235);
}
.studio-pulse-tooltip-line {
  display: block;
  font-size: 11px;
  line-height: 1.45;
  color: rgb(75 85 99);
}
:global(.dark) .studio-pulse-tooltip-line {
  color: rgb(209 213 219);
}
.studio-pulse-tooltip-title {
  margin-bottom: 0.2rem;
  font-weight: 600;
  color: rgb(17 24 39);
}
:global(.dark) .studio-pulse-tooltip-title {
