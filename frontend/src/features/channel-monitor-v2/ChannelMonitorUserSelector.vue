  ...
  <div ref="containerRef" class="relative">
    <div v-if="selectedUserIds.length > 0" class="mb-2 flex flex-wrap gap-2">
      ...
      >
        ...
        >
          {{ t('admin.channelMonitor.visibility.userDeleted') }}
        </span>
        <button
          type="button"
          class="shrink-0 rounded text-gray-400 hover:text-red-600 dark:hover:text-red-400"
          :aria-label="t('admin.channelMonitor.visibility.removeUser')"
          :title="t('admin.channelMonitor.visibility.removeUser')"
    ...
    <div class="relative">
      ...
      <input
        ...
        :placeholder="t('admin.channelMonitor.visibility.userSearchPlaceholder')"
    ...
    >
      ...
      >
        {{ t('admin.channelMonitor.visibility.userSearchEmpty') }}
      </div>
      <template v-else>
        <button
...
function selectedUserLabel(userId: number): string {
  return selectedUsers.value[userId]?.email ||
    t('admin.channelMonitor.visibility.userIdFallback', { id: userId })
}
