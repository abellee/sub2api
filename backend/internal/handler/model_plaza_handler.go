...
func (h *ModelPlazaHandler) get(c *gin.Context, enforceFeature bool) {
	...
	if enforceFeature {
		...
		groups, err = h.channelService.ListConfiguredPlazaGroups(c.Request.Context())
	}
	if err != nil {
...
func toModelPlazaOfficialPricing(p *service.PlazaOfficialPricing) *modelPlazaOfficialPricing {
	...
	return &modelPlazaOfficialPricing{
		...
		Intervals:         toModelPlazaPricingIntervals(p.Intervals),
	}
}

func toModelPlazaPricingIntervals(src []service.PricingInterval) []userPricingIntervalDTO {
	if src == nil {
		return nil
	}
